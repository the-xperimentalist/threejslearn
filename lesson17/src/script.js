...
// baseWalls
const baseWalls = new THREE.Mesh(
...
baseWalls.geometry.setAttribute(
    'uv2',
    new THREE.Float32BufferAttribute(baseWalls.geometry.attributes.uv.array, 2))
baseWalls.position.y = 2.5 / 2
house.add(baseWalls)
...
const tick = () =>
{
    ...
    // Update ghosts
    const ghostingAngle = elapsedTime
    ghost1.position.x = Math.cos(ghostingAngle) * (5 + Math.sin(ghostingAngle * 0.3))
    ghost1.position.z = Math.sin(ghostingAngle) * (5 + Math.cos(ghostingAngle * 0.4))
    ghost1.position.y = Math.sin(ghostingAngle * 2)

    ghost2.position.x = Math.cos(-ghostingAngle) * (6 + Math.sin(ghostingAngle * 0.5))
    ghost2.position.z = Math.sin(-ghostingAngle) * (6 + Math.cos(ghostingAngle * 0.45))
    ghost2.position.y = Math.cos(ghostingAngle)/2 + Math.cos(ghostingAngle * 2)/2

    ghost3.position.x = Math.cos(ghostingAngle) * (8 + Math.sin(ghostingAngle * 0.5))
    ghost3.position.z = Math.sin(ghostingAngle) * (8 + Math.cos(ghostingAngle * 0.45))
    ghost3.position.y = Math.cos(ghostingAngle*3) + Math.cos(ghostingAngle * 2)
