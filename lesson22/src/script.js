...
debugObject.resetButton = () => {
    for (const obj of objsToUpdate) {

        // Remove body from world
        obj.body.removeEventListener('collide', playHitSound)
        world.removeBody(obj.body)

        // Remove object from threejs scene
...
/**
 * Cannon world(Physics world) setup
 */
...
const defaultPlasticContactMaterial = new CANNON.ContactMaterial(
    ...
    plasticMaterial,
    {
        friction: 0.1, // Refers to the ground rub resistance
        restitution: 0.7 // Refers to the bouncing capabilities
    })
...
world.defaultContactMaterial = defaultContactMaterial // Works as a default for all the objects in the space, when specification is not required

// // Sphere
// const sphereShape = new CANNON.Sphere(0.5) // Same as my sphere radius
...
const createSphere = (radius, position) => {
    ...
    // Save to objects to update array
    objsToUpdate.push({
        mesh: mesh,
        body: body
    })
...
const createBox = (length, width, height, position) => {
    ...
    const shape = new CANNON.Box(new CANNON.Vec3(length * 0.5, width * 0.5, height * 0.5))
